...
import shortText from "../../lib/shortText";

const AdminAssignment = () => {
  const { data: assignments, isLoading, isError } = useGetAssignmentsQuery();
  const [deleteAssignment, { isLoading: isDeleteAssignLoading }] =
    useDeleteAssignmentMutation();
  ...
  if (!isLoading && isError) {
    content = <Error message="couldn't load data due to network error." />;
  }
      ...
      <table className="divide-y-1 text-base divide-gray-600 w-full">
        ...
        <tbody className="divide-y divide-slate-600/50">
          {assignments?.map((assignment) => {
            const { id, video_id, video_title, title, totalMark } =
              assignment || {};

            return (
              <tr>
                <td className="table-td">{shortText(title, 9)}</td>
                <td className="table-td">{shortText(video_title, 9)}</td>
                <td className="table-td">{totalMark}</td>
                <td className="table-td flex gap-x-2">
                  <button
                    disabled={isDeleteAssignLoading}
